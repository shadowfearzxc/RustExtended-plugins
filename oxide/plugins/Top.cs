...
{
    [Info("Top", "edited by kustanovich", "1.0.0")]
    class Top : RustLegacyPlugin	 
	{
		...
		void top(NetUser netuser, string command, string[] args)		
		{
            ...
            for (int n = 1; n <= 5; n++)
            {
				...
				float KD = RustExtended.Economy.Get(topPlayer.SteamID).PlayersKilled / RustExtended.Economy.Get(topPlayer.SteamID).Deaths;
		...
		{			
			...
			string KD = "КД: [COLOR#388FFF]" + RustExtended.Economy.Get(topPlayer.SteamID).PlayersKilled / RustExtended.Economy.Get(topPlayer.SteamID).Deaths + "[color#ffff00]";

			SendMessage(netuser.playerClient, string.Format("{0}", plkil));
			SendMessage(netuser.playerClient, string.Format("{0}", death));	
SendMessage(netuser.playerClient, string.Format("{0}", KD));			
		}
